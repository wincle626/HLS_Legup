...
main ()
{
      ...
      word *SHARED_so = malloc_shared(sizeof(word) * N, so, LEGUP_RAM_LOCATION_ONCHIP);
      word *SHARED_LARc = malloc_shared(sizeof(word) * M, LARc, LEGUP_RAM_LOCATION_ONCHIP);
	...
	so[i] = inData[i];
      ...
      memcpy_to_shared(SHARED_so, so, sizeof(word) * N);

      Gsm_LPC_Analysis (SHARED_so, SHARED_LARc);

      memcpy_from_shared(so, SHARED_so, sizeof(word) * N);
      memcpy_from_shared(LARc, SHARED_LARc, sizeof(word) * M);

      free_shared(SHARED_so);
      free_shared(SHARED_LARc);
