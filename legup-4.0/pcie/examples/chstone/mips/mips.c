...
void
mips_main (int *dmem_in, int dmem_len, int *dmem, int *imem, int *n_inst_out)
{
	long long hilo;
	int reg[32];
	int Hi = 0;
...
const unsigned long imem[44] = {
  ...
  0x03e00008,			// [0x004000ac]  jr $31                          ; 38: jr   $ra
      ...
      int *SHARED_A = malloc_shared(64 * sizeof(int), A, LEGUP_RAM_LOCATION_ONCHIP);
      int *SHARED_dmem_out = malloc_shared(64 * sizeof(int), dmem_out, LEGUP_RAM_LOCATION_ONCHIP);
      int *SHARED_imem = malloc_shared(44 * sizeof(int), imem, LEGUP_RAM_LOCATION_ONCHIP);
      int *SHARED_n_inst_ptr = malloc_shared(sizeof(int), &n_inst, LEGUP_RAM_LOCATION_ONCHIP);

      memcpy_to_shared(SHARED_A, A, 64 * sizeof(int));
      memcpy_to_shared(SHARED_imem, imem, 44 * sizeof(int));

      mips_main(SHARED_A, 64, SHARED_dmem_out, SHARED_imem, SHARED_n_inst_ptr);

      memcpy_from_shared(dmem_out, SHARED_dmem_out, 8 * sizeof(int));
      memcpy_from_shared(&n_inst, SHARED_n_inst_ptr, sizeof(int));

      free_shared(SHARED_A);
      free_shared(SHARED_dmem_out);
      free_shared(SHARED_imem);
      free_shared(SHARED_n_inst_ptr);

      main_result += (n_inst == 611);
      for (j = 0; j < 8; j++)
	{
	  main_result += (dmem_out[j] == outData[j]);
	}
