...
main ()
{
  ...
  int s, motion_vector_count, mv_format, h_r_size, v_r_size, dmv, mvscale;
      ...
      for (i = 0; i < 2; i++)
	{
      ...
      int *SHARED_PMV = malloc_shared(2 * 2 * 2 * sizeof(int), PMV, LEGUP_RAM_LOCATION_ONCHIP);
      int *SHARED_dmvector = malloc_shared(2 * sizeof(int), dmvector, LEGUP_RAM_LOCATION_ONCHIP);
      int *SHARED_mvfs = malloc_shared(2 * 2 * sizeof(int), motion_vertical_field_select, LEGUP_RAM_LOCATION_ONCHIP);

      memcpy_to_shared(SHARED_PMV, PMV, 2 * 2 * 2 * sizeof(int));
      memcpy_to_shared(SHARED_dmvector, dmvector, 2 * sizeof(int));
      memcpy_to_shared(SHARED_mvfs, motion_vertical_field_select, 2 * 2 * sizeof(int));

      motion_vectors (SHARED_PMV, SHARED_dmvector, SHARED_mvfs, s,
		      motion_vector_count, mv_format, h_r_size, v_r_size, dmv,
		      mvscale);

      memcpy_from_shared(PMV, SHARED_PMV, 2 * 2 * 2 * sizeof(int));
      memcpy_from_shared(motion_vertical_field_select, SHARED_mvfs, 2 * 2 * sizeof(int));

      free_shared(SHARED_PMV);
      free_shared(SHARED_dmvector);
      free_shared(SHARED_mvfs);
