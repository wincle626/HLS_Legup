...
#ifdef SW_ONLY
#define OFFSET (i*xpartition*3*arg->yres)
#else
#define OFFSET (0)
#endif
    ...
    for (i = 0; i < loop_itr; i++) {
        ...
        generate_set(xstart, xend, arg->xmax, arg->xmin, arg->ymax, arg->ymin, arg->xres, arg->yres, arg->itr, arg->SHARED_MEM_ptr + 3*arg->yres*arg->thread_id*row_size + OFFSET, arg->thread_id);

        // post-copy
        memcpy_from_shared(arg->ptr + 3*arg->yres*xstart, arg->SHARED_MEM_ptr + 3*arg->yres*arg->thread_id*row_size, 3*arg->yres*(xend-xstart));
    }
...
void generate_mandelbrot(int itr, int xres, int yres, IplImage *img)
{
    ...
    // pre-allocate
    char *SHARED_MEM_ptr = malloc_shared((3*xres*yres) >> dividend, ptr, LEGUP_RAM_LOCATION_ONCHIP);
...
int main()
{
  ...
  IplImage *img1 = cvCreateImage(cvSize(xres, yres), IPL_DEPTH_8U, 3);
  IplImage *img2 = cvCreateImage(cvSize(xres, yres), IPL_DEPTH_8U, 3);
