...
void generate_set(int xstart, int xend, int xmax, int xmin, int ymax, int ymin,
        int xres, int yres, int itr, int *ptr, volatile int thread_id)
{
...
#define OFFSET (0)
    ...
    for (i = 0; i < loop_itr; i++) {
        ...
        generate_set(xstart, xend, arg->xmax, arg->xmin, arg->ymax, arg->ymin, arg->xres, arg->yres, arg->itr, arg->SHARED_MEM_ptr + arg->yres*arg->thread_id*row_size + OFFSET, arg->thread_id);
