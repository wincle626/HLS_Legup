...
// 	dumpExampleDependence(OS, F, const_cast<DependenceAnalysis *>(this));
	...
	DependenceAnalysis *DA = const_cast<DependenceAnalysis *>(this);
	/* example of checking loop dependency */
	OS << "########## Checking Loop Dependency in Single-BB Innermost Loops: ###################\n";
	std::vector<Loop *> innermostLoops;
	std::deque<Loop *> loopsToCheck;
	for (LoopInfo::iterator lpi = LI->begin(), lpe = LI->end();
			lpi != lpe; lpi++) {
		loopsToCheck.push_back( *lpi );
	}
	OS<< "### Number of Top-level Loops: "<<loopsToCheck.size()<<"\n";

	while (loopsToCheck.size() != 0 ) {
		Loop* lp = loopsToCheck.front();
		loopsToCheck.pop_front();
		std::vector< Loop* > subLoops = lp->getSubLoops();
		// check all the subLoops later
		for (std::vector< Loop * >::iterator lpi = subLoops.begin(), lpe = subLoops.end();
				lpi != lpe; loopsToCheck.push_back(*lpi), lpi++);
		// if the loop has only one BB
		if (lp->getNumBlocks() == 1) {
			assert (subLoops.size() == 0);
			innermostLoops.push_back(lp);
		}
	}
	OS<< "### Number of Single-BB Innermost Loops: "<<innermostLoops.size()<<"\n";

	int cnt = 0;
	for (std::vector< Loop * >::iterator lpi = innermostLoops.begin(), lpe = innermostLoops.end();
			lpi != lpe; lpi++) {
		cnt ++;
		OS << "\n########## Innermost Loop - "<<cnt<<"###################\n";
		assert ((*lpi)->getNumBlocks() == 1);

		BasicBlock *bb = *( (*lpi)->block_begin() );
		BasicBlock::iterator e = bb->end();
		for (BasicBlock::iterator SrcI = bb->begin(); SrcI != e; SrcI++) {
			if (isa<StoreInst>(*SrcI) || isa<LoadInst>(*SrcI)) {
				for (BasicBlock::iterator DstI = SrcI; DstI != e; DstI ++) {
					if (isa<StoreInst>(*DstI) || isa<LoadInst>(*DstI)) {
						OS << "Src: ";
						SrcI->print(OS);
						OS << "\tDst: ";
