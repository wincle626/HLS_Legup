...
/*
	...
	assign cacheClkEnOriginal = (memRead && (state == stateIDLE || (state == stateREAD && cacheHit)) && !bypassCache)
	|| fetchDone || (memWrite && state == stateIDLE && !bypassCache) || (state == stateWRITE && cacheHit)
	|| state == stateFLUSH;
	...
	assign cacheClkEn = cacheClkEnOriginal | cacheClkEnDelayed;	
...
*/	
	...
	//assign bypassCache = 1'b1; //temporary change to fix cache coherency problem
