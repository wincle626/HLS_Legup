...
////////////////////////////////	
	...
	assign aso_CachetoTiger_data[37] = cacheHit;
	assign aso_CachetoTiger_data[39:38] = 2'b0;
	/////////////////////////////////////////////////////////////
...
assign cacheAddress = fetchDone ? address[cacheSize + blockSize - 1 : blockSize] :
	...
	reg memWrite_reg;
	reg fetchDone_reg_accel;
	reg fetchDone_reg;

	always @(posedge clk)
	begin
		memWrite_reg <= memWrite;
		fetchDone_reg_accel <= fetchDone_accel;
...
/*	
	...
	|| state_accel == stateFLUSH || fetchDone_reg_accel;
	...
	|| state == stateFLUSH || fetchDone_reg;
*/
...
/*
	assign avs_ACCEL_waitrequest = state_accel == stateFETCH 
//								|| (memRead_accel && (state_accel != stateAVALON_READ) && !cacheHit_accel)
								|| (memRead_accel && avs_ACCEL_begintransfer)
								|| (state_accel == stateREAD && !cacheHit_accel)
								|| (state_accel == stateAVALON_READ && !avm_AccelMaster_readdatavalid)
								|| (memWrite_accel && state_accel != stateIDLE && state_accel != stateREAD) || (state_accel != stateREAD && state_accel != stateIDLE && memRead_accel)
								|| (state_accel == stateHOLD)
								|| state_64 == 1 || state_64 == 2;
*/
	assign avs_ACCEL_waitrequest = state_accel == stateFETCH //while fetching from off-chip
								|| state_accel == stateHOLD //before going into fetch state
								|| memRead_accel && avs_ACCEL_begintransfer //when accel first accesses cache
								|| (memRead_accel && !cacheHit_accel)// || (memRead_accel && state_accel != stateREAD) //if on a read, it's not a cache hit, and it's not in stateREAD
								|| (memWrite_accel && state_accel != stateWRITE) //if on a write, it's not in stateWRITE
								|| avm_AccelMaster_waitrequest //if waitrequest is asserted from off-chip memory
								|| state_64 == 1 || state_64 == 2; //for 64-bit accesses
