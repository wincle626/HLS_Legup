...
int main(int argc, char** argv) {
	...
	if (argc != 3) { printf("Incorrect usage, expected: %s  <file.elf> <NUM_FUNCTIONS>.\n", argv[0]); exit(1); }
	sscanf(argv[2], "%d", &NUM_FUNCTIONS);
	...
	if (fDump.is_open()) {
		...
		while (!fDump.eof()) {
			if (line.size() > 0) {
				if (line.size() > 8 && line.substr(8,2) == " <") { //line[0] == '0') {
	...
	int last_line2;
	char last_line3[10];
	sscanf(last_line.c_str(), " %x: ", &last_line2);
	sprintf(last_line3, "%08X", last_line2);
	printf("last_line2 = %s\n", last_line3);
	functions[functions.size()-1].end = offsetHex(last_line3, 0);

	// remove file extension
	string basename = string(argv[1]);
	basename = basename.substr(0,basename.find_last_of('.'));
	FILE *flist = fopen((basename+".flist_range").c_str(), "w");
