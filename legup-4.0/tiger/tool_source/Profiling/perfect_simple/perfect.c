...
key      *keys;                                      /* head of list of keys */
{
	...
	for (i=0; i<final->used; ++i) fprintf(f, final->line[i]);
...
void make_prof (ub4 blen, ub4 nmax, ub4 *scramble, bstuff *tab) {
	...
	fprintf(out, "tab[] = {");
	for (i=0; i<nmax; i++) {
		if (i<blen)	fprintf(out, "%d,",scramble[tab[i].val_b]);
		else		  	fprintf(out, "0,");
	}
	fprintf(out, "}\n");
