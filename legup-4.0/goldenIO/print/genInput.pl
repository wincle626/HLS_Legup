...
while (<FILE>) {
	...
	for ($j = 0; $j < $num_arg; $j++) {
        ...
        # checks for constant, limit and array specifier in that order
		@const_check = split(/=/, $tokens[$j]);
		@limit_check = split(/</, $const_check[0]);  #used later in # generation
	...
	#printf and limit holds the flag used in printf and limit used to generate random input respectively.  
		if (($char == 1)&&($array_count[$count] == 1)){
			...
			$printf[$count] = "f";
			$limit[$count] = $float_limit;	
			if ($array_count[$count] == 1){
				print "	float arg$count;\n";
			}
			else {
				print "	float arg$count\[$array_count[$count]\];\n";
			}
		}
		elsif ($double == 1){
			$printf[$count] = "f";
			$limit[$count] = $double_limit;	
			if ($array_count[$count] == 1){
				print "	double arg$count;\n";
			}
			else {
				print "	double arg$count\[$array_count[$count]\];\n";
			}
		}
...
#check output type.  Same logic as input.
		...
		elsif (($output[0] eq "float")||($output[0] eq "double")){
			$out_printf = "f";
		}
		else{
	...
	for ($i = 0; $i < $num_test; $i++) {
		...
		#number
			...
			#character or string
				...
				if ($array_count[$a] == 1){
					print "	arg$a = \'$string\'; /*type: $printf[$a]*/\n";  #single quote for character
				}
				else{
