...
// The full 222 term sequence, starting with seed[0] (77031) is:
...
// Every 8th term, starting from the initial seed:
...
// Pre-calculated first 8 sequence values.
// Starting with this, we can calculate the entire series 8 at a time.
volatile unsigned int seeds [] = {77031, 115547, 173321, 259982, 
                                 129991, 194987, 292481, 438722};
...
int main () {
    // 222 / 8 = 27.75, or 27 rem 6
    // 28 * 8 = 224
    // So two extra values ("2 1") at end.
 loop: for (int i = 0; i < 28; i++) {
        ...
        // This loop should unroll or vectorize well!
        for (int j = 0; j < 8; j++) {
            seeds[j] = hailstone_every_8th(seeds[j]);
        }
	//        printf("\n");
    }
    return seeds[7];
