...
l5:		for (i = 0; i <= MAXGRID - 1; i++) {
			path[0][i] = mean[0][i];
		}

		for (j = 1; j <= MAXGRID - 1; j++) {
l6:		for (i = j; i <= MAXGRID - 1; i++) {
				path[j][i] = path[j - 1][i - 1] + mean[j][i];
			}
		}

// #define min(x,y) ( x<y ? x : y )
// 		for (i = 2; i <= 2*MAXGRID-2; i++) {
// l6:			for (j = 1+(i-1)/2; j <= min(MAXGRID-1, i-1); j++) {
// 				path[i-j][j] = path[i-j-1][j-1] + mean[i-j][j];
// 			}
// 		}
 	}
#pragma endscop
