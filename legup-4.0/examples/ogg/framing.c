...
void checkpacket(ogg_packet *op,int len, int no, int pos){
  ...
  if(op->bytes!=len){
    printf("incorrect packet length (%d != %d)!\n",op->bytes,len);
    exit(1);
  }
  if(op->granulepos!=pos){
    printf("incorrect packet granpos (%ld != %d)!\n",(long)op->granulepos,pos);
    exit(1);
  }
  ...
  if(op->packetno!=sequence){
    printf("incorrect packet sequence %ld != %d\n",
  ...
  for(j=0;j<op->bytes;j++)
    if(op->packet[j]!=((j+no)&0xff)){
      printf("body data mismatch (1) at pos %ld: %x!=%lx!\n\n",
...
void check_page(unsigned char *data,const int *header,ogg_page *og){
  ...
  for(j=0;j<og->body_len;j++)
    if(og->body[j]!=data[j]){
      printf("body data mismatch (2) at pos %ld: %x!=%x!\n\n",
              j,data[j],og->body[j]);
      exit(1);
    }
  ...
  for(j=0;j<og->header_len;j++){
    if(og->header[j]!=header[j]){
      printf("header content mismatch at pos %ld:\n",j);
      for(j=0;j<header[26]+27;j++)
        printf(" (%ld)%02x:%02x",j,header[j],og->header[j]);
      printf("\n");
      exit(1);
    }
  }
  if(og->header_len!=header[26]+27){
    printf("header length incorrect! (%ld!=%d)\n",
...
void print_header(ogg_page *og){
  int j;
  printf("\nHEADER:\n");
  printf("  capture: %c %c %c %c  version: %d  flags: %x\n",
          og->header[0],og->header[1],og->header[2],og->header[3],
          (int)og->header[4],(int)og->header[5]);

  printf("  granulepos: %d  serialno: %d  pageno: %ld\n",
...
void test_pack(const int *pl, const int **headers, int byteskip,
          ...
          while(1){
            ...
            while(ogg_stream_packetpeek(&os_de,&op_de2)>0){
              ...
              if(memcmp(&op_de,&op_de2,sizeof(op_de))){
                printf("packetout != packetpeek! pos=%ld\n",
                        depacket);
                exit(1);
              }
              ...
              if(memcmp(data+depacket,op_de.packet,op_de.bytes)){
                printf("packet data mismatch in decode! pos=%ld\n",
                        depacket);
                exit(1);
              }
              /* check bos flag */
              if(bosflag==0 && op_de.b_o_s==0){
                printf("b_o_s flag not set on packet!\n");
                exit(1);
              }
              if(bosflag && op_de.b_o_s){
                printf("b_o_s flag incorrectly set on packet!\n");
                exit(1);
              }
              bosflag=1;
              ...
              /* check eos flag */
              if(eosflag){
                printf("Multiple decoded packets with eos flag!\n");
                exit(1);
              }
              ...
              if(op_de.granulepos!=-1){
                printf(" granule:%ld ",(long)op_de.granulepos);
              }
            }
          }
        }
  ...
  _ogg_free(data);
  if(headers[pageno]!=NULL){
    printf("did not write last page!\n");
    exit(1);
  }
  if(headers[pageout]!=NULL){
    printf("did not decode last page!\n");
    exit(1);
  }
  if(inptr!=outptr){
    printf("encoded page data incomplete!\n");
    exit(1);
  }
  if(inptr!=deptr){
    printf("decoded page data incomplete!\n");
    exit(1);
  }
  if(inptr!=depacket){
    printf("decoded packet data incomplete!\n");
    exit(1);
  }
  if(!eosflag){
    printf("Never got a packet with EOS set!\n");
    exit(1);
  }
  printf("ok.\n");
}
...
int framing_main(void){
...

  {
    ...
    printf("testing single page encoding... ");
...

  {
    ...
    printf("testing basic page encoding... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing basic nil packets... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing initial-packet lacing > 4k... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing single packet page span... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing page spill expansion... ");
    test_pack(packets,headret,0,0,0);
  }
  ...
  /* page with the 255 segment limit */
  {
    ...
    printf("testing max packet segments... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing very large packets... ");
    test_pack(packets,headret,0,0,0);
  }

  {
    ...
    printf("testing continuation resync in very large packets... ");
...

  {
    ...
    printf("testing zero data page (1 nil packet)... ");
    test_pack(packets,headret,0,0,0);
  }
...

  {
    ...
    for(i=0;i<5;i++){
      if(ogg_stream_pageout(&os_en,&og[i])==0){
        printf("Too few pages output building sync tests!\n");
        exit(1);
      }
      copy_page(&og[i]);
    }
    ...
    /* Test lost pages on pagein/packetout: no rollback */
    {
      ...
      if(ogg_stream_packetout(&os_de,&test)!=-1){
        printf("Error: loss of page did not return error\n");
        exit(1);
      }
      if(ogg_stream_packetout(&os_de,&test)!=1)error();
    ...
    /* Test lost pages on pagein/packetout: rollback with continuation */
    {
      ...
      printf("Testing loss of pages (rollback required)... ");
    ...
    /* the rest only test sync */
    {
      ...
      printf("Testing sync on partial inputs... ");
      ogg_sync_reset(&oy);
    ...
    {
      ogg_page og_de;
      printf("Testing sync on 1+partial inputs... ");
      ogg_sync_reset(&oy);
    ...
    {
      ogg_page og_de;
      printf("Testing search for capture... ");
      ogg_sync_reset(&oy); 
