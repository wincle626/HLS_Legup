...
int8 float_exception_flags_add[OMP_ACCEL] = {0};
int8 float_exception_flags_mul[OMP_ACCEL] = {0}; 
int8 float_exception_flags_div[OMP_ACCEL] = {0}; 
int8 float_exception_flags_sin[OMP_ACCEL] = {0}; 
...
roundAndPackFloat64 (flag zSign, int16 zExp, bits64 zSig, int type)
{
  ...
  if (roundBits){
      ...
      if (type == 0)
        float_exception_flags_add[tid] |= float_flag_inexact;
      if (type == 1)
        float_exception_flags_mul[tid] |= float_flag_inexact;
      if (type == 2)
        float_exception_flags_div[tid] |= float_flag_inexact;
      if (type == 3)
        float_exception_flags_sin[tid] |= float_flag_inexact;
  }
