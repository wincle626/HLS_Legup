...
__attribute__((noinline)) void pthread_fork_1(pthread_t *threads, int *result) {
...
__attribute__((noinline)) void pthread_fork_2(pthread_t *threads, int *result) {

    pthread_create(&threads[2], NULL, div, NULL);
    pthread_create(&threads[3], NULL, dfsin, NULL);

    pthread_join(threads[2], (void **)&result[2]);
    pthread_join(threads[3], (void **)&result[3]);
}
...
int main() {
    ...
    pthread_fork_1(threads, result);
    pthread_fork_2(threads, result);
