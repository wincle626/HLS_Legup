...
loop7:    for(mptr=matrix;mptr<matrix+DCTSIZE2;mptr++){
        *mptr = *mptr*(*qmatrix);
        qmatrix++;
    }
}
...
{
    int *mptr;
loop8:    for(mptr=matrix; mptr < matrix + DCTSIZE2; mptr++) {
        *mptr += shift;
    }
}
...
loop9:    for(mptr=matrix; mptr < matrix + DCTSIZE2; mptr++){
//        if (*mptr < 0) {
//            *mptr = 0;
//        }
//        else if (*mptr > Bound) {
...
decode_start(int* out_data_image_width, int* out_data_image_height, int* out_data_comp_vpos, int* out_data_comp_hpos)
{
     ...
     */
loop11:    for(i = 0; i < NUM_COMPONENT; i++){
        HuffBuff[i][0] = 0;
    }
