...
void EdmondMatching::initGraph(int n){
     for (int i=0; i<n; i++)
         for (int j=0; j<n; j++) g[i][j]=noEdge;
}

int EdmondMatching::readGraph(){
     int n,e,a,b;
     int result = scanf(" %d %d",&n,&e);      //The graph is read and its edges are unmatched by default.
     assert(result);
...
void EdmondMatching::initAlg(int n){             //Initializes all data structures for the augmentMatching()
...
void EdmondMatching::backtrace (int vert, int pathNum, int stop, int parity, int direction){
...
void EdmondMatching::enqueue (int vert, int t){
...
void EdmondMatching::newBlossom (int a, int b){     //newBlossom() will be called after the paths are evaluated.
...
void EdmondMatching::augmentPath (){           //An augmenting path has been found in the matching
...
void EdmondMatching::findMaximumMatching (int n){
